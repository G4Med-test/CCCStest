                           ...
                           G4String symbol, G4double density, G4int Z, G4int A)
{
 ...
 G4Element* element  = new G4Element(name, symbol, 1);
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  G4NistManager* man = G4NistManager::Instance();
  G4Material *worldMat = man->FindOrBuildMaterial("G4_AIR");

  G4Box*
  sBox = new G4Box("World",                         //its name
                   fBoxSize/2*1.01,fBoxSize/2*1.01,fBoxSize/2*1.01);   //its dimensions
  ...
  G4Box *sDet = new G4Box("Detector",                         //its name
                   fBoxSize/2*1.01,fBoxSize/2*1.01,fBoxSize/2*1.01);   //its dimensions

  fLDet = new G4LogicalVolume(sDet,                     //its shape
                             fMaterial,                 //its material
                             "DetLog");     //its name

  fPDet = new G4PVPlacement(0,                          //no rotation
                            G4ThreeVector(),            //at (0,0,0)
                            fLDet,                      //its logical volume
  ...
  fLDet->SetRegion(aRegion);
  aRegion->AddRootLogicalVolume(fLDet);
